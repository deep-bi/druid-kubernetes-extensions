...
public class DefaultK8sApiClient implements K8sApiClient {
        ...
        try {
            PatchUtils.patch(
                    ...
                    () -> coreV1Api.patchNamespacedPodCall(
                            podName, podNamespace, new V1Patch(jsonPatchStr), "true", null, null, null, null, null),
    ...
    public DiscoveryDruidNodeList listPods(String podNamespace, String labelSelector, NodeRole nodeRole) {
        try {
            V1PodList podList = coreV1Api.listNamespacedPod(
                    podNamespace, null, null, null, null, labelSelector, null, null, null, null, null, null);
            ...
            for (V1Pod podDef : podList.getItems()) {
                if (!isPodReady(podDef)) {
                    LOGGER.info(
                            ...
                            podDef.getMetadata().getName(), nodeRole);
                    continue;
                }
    ...
    @Override
    public WatchResult watchPods(
            String namespace, String labelSelector, String lastKnownResourceVersion, NodeRole nodeRole) {
        try {
            Watch<V1Pod> watch = Watch.createWatch(
                    ...
                    coreV1Api.listNamespacedPodCall(
                            ...
                            true,
                            null),
                    new TypeReference<Watch.Response<V1Pod>>() {}.getType());
                                ...
                                if (item.object != null) {
                                    if (!isPodReady(item.object)) {
                                        if (WatchResult.MODIFIED.equals(item.type)) {
                                            ...
                                            // It will appear via a MODIFIED event that remaps to ADDED for discovery,
                                        ...
                                        // This is safe even if the node is already in the cache because
                                        // BaseNodeRoleWatcher.childAdded() uses
                                    ...
                                    try {
                                        ...
                                        LOGGER.warn(
                                                ...
                                                nodeRole,
                                                item.type);
                                        // result stays null, caller will restart the watch and do a full listPods
                                        // resync
                                    }
                                } else {
                                    ...
                                    LOGGER.debug(
                                            "item of type " + item.type + " was NULL when watching nodeRole [%s]",
                                            nodeRole);
                                }

                                obj = new Watch.Response<>(effectiveType, result);
                                return true;
