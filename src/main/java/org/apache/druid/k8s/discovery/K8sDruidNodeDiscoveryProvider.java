...
public class K8sDruidNodeDiscoveryProvider extends DruidNodeDiscoveryProvider {
    ...
    public K8sDruidNodeDiscoveryProvider(
            PodInfo podInfo, K8sDiscoveryConfig discoveryConfig, K8sApiClient k8sApiClient) {
    ...
    public BooleanSupplier getForNode(DruidNode node, NodeRole nodeRole) {
        return () -> k8sApiClient
                .listPods(
                        ...
                        nodeRole)
                .getDruidNodes()
                .containsKey(node.getHostAndPortToUse());
    }
    ...
    NodeRoleWatcher getForNodeRole(NodeRole nodeType, boolean startAfterCreation) {
        ...
        return nodeTypeWatchers.computeIfAbsent(nodeType, nType -> {
            LOGGER.info("Creating NodeRoleWatcher for role[%s].", nType);
            NodeRoleWatcher nodeRoleWatcher = new NodeRoleWatcher(
                    listenerExecutor, nType, podInfo, discoveryConfig, k8sApiClient, watcherErrorRetryWaitMS);
    ...
    public void start() {
        ...
        try {
            ...
            // This is single-threaded to ensure that all listener calls are executed precisely in the oder of
            // add/remove
            // event occurences.
    ...
    static class NodeRoleWatcher implements DruidNodeDiscovery {
        ...
        private void watch() {
            ...
            while (lifecycleLock.awaitStarted(1, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    keepWatching(labelSelector, list.getResourceVersion());
                } catch (Throwable ex) {
        ...
        private void keepWatching(String labelSelector, String resourceVersion) {
            ...
            while (lifecycleLock.awaitStarted(1, TimeUnit.MILLISECONDS)) {
                ...
                try (WatchResult iter = k8sApiClient.watchPods(
                        podInfo.getPodNamespace(), labelSelector, nextResourceVersion, nodeRole)) {
                    if (iter == null) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (item == null || item.type == null || item.object == null) {
                            LOGGER.debug(
                                    "Received NULL item while watching role[%s]. Restarting watch.", this.nodeRole);
                            return;
                        }
                        ...
                        switch (item.type) {
                            ...
                            case WatchResult.NOT_READY:
                                ...
                                // DELETED can fire after NOT_READY (so the service is already removed), or
                                // before ADDED (pod deleted before becoming ready).
                                // NOT_READY can repeat during CrashLoopBackOff. None of these warrant the
